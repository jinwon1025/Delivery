...
public class NoticeController {
	...
	public ModelAndView ownerNoticeList(HttpSession session, Integer PAGE_NUM) {
		...
		Integer count = this.noticeService.getMaxCountFromOwnerNotice();
		int startRow = 0;
		int endRow = 0;
		int totalPageCount = 0;
		if (count > 0) {
		...
		List<OwnerNotice> noticeList = noticeService.getAllOwnerNotices(se);
		mav.addObject("START", startRow);
		mav.addObject("END", endRow);
		mav.addObject("TOTAL", count);
		mav.addObject("currentPage", currentPage);
	...
	public ModelAndView adminUserNoticeList(HttpSession session, Integer PAGE_NUM) {
		...
		if (count > 0) {
			...
			endRow = ((currentPage - 1) * 5) + 6; // 기존 코드
	...
	public ModelAndView userNoticeWrite(UserNotice notice, HttpSession session) {
		...
		Integer maxCount = this.noticeService.getMaxCountFromUserNotice2();
		if (maxCount == null) {
			maxCount = 0;
		}

		notice.setNotice_id(maxCount + 1);
	...
	public ModelAndView adminOwnerNoticeList(HttpSession session, Integer PAGE_NUM) {
		ModelAndView mav = new ModelAndView("admin/adminMain");
	...
	public ModelAndView ownerNoticeWrite(OwnerNotice notice, HttpSession session) {
		...
		Integer maxCount = this.noticeService.getMaxCountFromOwnerNotice2();
		if(maxCount == null) {
			maxCount=0;
		}
		notice.setNotice_id(maxCount+1);
