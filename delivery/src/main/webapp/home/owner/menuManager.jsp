		...
		<div class="store-container">
						...
						<c:otherwise>
							...
							<c:forEach items="${menuList}" var="category" varStatus="status">
								<div class="menu-category ${status.first ? 'active' : ''}">
									<div
										class="menu-category-header d-flex justify-content-between align-items-center p-3 bg-gray-100 rounded">
										<h3 class="m-0 font-medium">${category.menu_category_name}</h3>
										<div class="d-flex align-items-center">
											<form action="/store/categoryUpdate" method="post"
												class="mr-2"
												onsubmit="return confirmCategoryUpdate(${category.menu_category_id});">
												<input type="hidden" name="menu_category_id"
													value="${category.menu_category_id}" /> <input
													type="hidden"
													id="menu_category_name_${category.menu_category_id}"
													name="menu_category_name"
													value="${category.menu_category_name}" />
												<button type="submit" class="btn btn-sm btn-primary">수정</button>
											</form>
											<form action="/store/categoryDelete" method="post"
												class="mr-2" onsubmit="return confirmCategoryDelete();">
												<input type="hidden" name="menu_category_id"
									...
									<div class="menu-items p-3">
										...
										<c:forEach items="${menuItemList}" var="menuItem">
											<c:if
												test="${menuItem.menu_category_id eq category.menu_category_id}">
										...
										<c:choose>
											<c:when test="${hasMenuItems eq 'false'}">
												...
												<div
													class="empty-menu-state p-4 text-center bg-gray-50 rounded border border-dashed border-gray-300 mb-3">
													<div class="empty-state-icon mb-3">
														<i class="fas fa-utensils fa-2x text-gray-400"></i>
													</div>
													<h4 class="font-medium mb-2">등록된 메뉴가 없습니다</h4>
													<p class="text-gray-500 mb-3">아래 버튼을 클릭하여 새로운 메뉴를 추가해
														보세요.</p>
												</div>
											</c:when>
											<c:otherwise>
																...
																<div class="menu-item-image mr-3">
																	<c:choose>
																		<c:when test="${not empty menuItem.image_name}">
																			<img
																				src="${pageContext.request.contextPath}/upload/menuItemProfile/${menuItem.image_name}"
																	...
																	<div class="menu-item-actions mt-2">
																		...
																		<form action="/store/menuDelete" method="post"
																			...
																			<input type="hidden" name="menu_item_id"
																				value="${menuItem.menu_item_id}" />
																			<button type="submit"
																				class="btn btn-sm btn-outline-gold">삭제</button>
																		</form>
																		<form action="/store/optionManage" method="get"
																			class="d-inline-block">
																			<input type="hidden" name="menu_item_id"
																				value="${menuItem.menu_item_id}" />
																			<button type="submit" class="btn btn-sm btn-white">옵션
																				관리</button>
																		</form>
																	</div>
																</div>
			...
			<div class="card">
				...
				<div class="card-body">
					...
					<form:form action="/store/categoryRegister"
						modelAttribute="menuCategory" method="post">
