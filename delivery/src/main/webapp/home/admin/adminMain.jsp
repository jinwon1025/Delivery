...
<body>
    ...
    <div class="admin-header">
        <div class="admin-logo">
            <i class="fas fa-utensils" style="font-size: 1.75rem; color: var(--primary-color);"></i>
            <h1>금베달리스트 관리자</h1>
        </div>
        <div class="admin-user">
            <c:choose>
                <c:when test="${not empty sessionScope.loginUser.image_name}">
                    <!-- 프로필 사진이 있을 때 -->
                    <img src="${pageContext.request.contextPath}/upload/userProfile/${sessionScope.loginUser.image_name}" alt="Profile" />
                </c:when>
                <c:otherwise>
                    <!-- 프로필 사진이 없을 때 -->
                    <i class="fas fa-user-circle" style="font-size: 1.75rem; color: var(--gray-500);"></i>
                </c:otherwise>
            </c:choose>
            <span>${sessionScope.loginUser.user_name}님</span>
            <a href="<c:url value='/user/logout'/>" class="btn btn-sm btn-secondary ml-3">로그아웃</a>
        </div>
    </div>
    ...
    <div class="admin-layout">
        ...
        <div class="admin-sidebar">
            <ul class="admin-menu">
                <li class="admin-menu-item ${activeMenu eq 'dashboard' ? 'active' : ''}" onclick="location.href='<c:url value='/admin/home'/>'">
                    <i class="fas fa-tachometer-alt"></i>
                    <span>대시보드</span>
                </li>
                <li class="admin-menu-item ${activeMenu eq 'userManagement' ? 'active' : ''}" onclick="location.href='<c:url value='/admin/userManagement'/>'">
                    <i class="fas fa-users"></i>
                    <span>회원관리</span>
                </li>
                <li class="admin-menu-item ${activeMenu eq 'categoryManagement' ? 'active' : ''}" onclick="location.href='<c:url value='/admin/categoryManagement'/>'">
                    <i class="fas fa-list"></i>
                    <span>카테고리관리</span>
                </li>
                <li class="admin-menu-item ${activeMenu eq 'couponManagement' ? 'active' : ''}" onclick="location.href='<c:url value='/admin/couponManagement'/>'">
                    ...
                    <div class="widget-container">
                        <div class="card">
                            <div class="card-header">
                                <h5 class="mb-0"><i class="fas fa-chart-bar"></i> 최근 거래 통계</h5>
                            </div>
                            <div class="card-body">
                                <p class="text-muted">최근 7일간의 주문 통계 그래프가 여기에 표시됩니다.</p>
                                <div class="text-center py-4">
                                    <i class="fas fa-chart-line" style="font-size: 4rem; color: var(--gray-300);"></i>
