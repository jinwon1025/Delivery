...
<head>
    ...
    <link rel="stylesheet" href="<c:url value='/css/user/user-components.css'/>">
...
<body>
    ...
    <header class="user-header">
        <div class="header-container">
            <div class="logo-area">
                <a href="<c:url value='/user/index'/>" class="site-logo">
                    <i class="fas fa-medal"></i>
                    <span class="logo-text">금베달리스트</span>
                </a>
            </div>
            ...
            <div class="user-account">
                <c:choose>
                    <c:when test="${sessionScope.loginUser != null}">
                        <div class="user-info">
                            <c:choose>
                                <c:when test="${not empty sessionScope.loginUser.image_name}">
                                    <img class="profile-pic" src="${pageContext.request.contextPath}/upload/userProfile/${sessionScope.loginUser.image_name}" alt="프로필 이미지" />
                                </c:when>
                                <c:otherwise>
                        ...
                        <a href="<c:url value='/user/mypage'/>" title="마이페이지">
                            <i class="fas fa-user-circle icon-link"></i>
                        </a>
                        <a href="<c:url value='/userstore/viewCart'/>" title="장바구니" class="icon-link cart">
                            <i class="fas fa-shopping-cart"></i>
                            <!-- 장바구니에 상품이 있는 경우 카운트 표시 -->
                            <c:if test="${cartCount > 0}">
                                <span class="cart-count">${cartCount}</span>
                            </c:if>
                        </a>
                        <a href="<c:url value='/user/bookMarkList'/>" title="즐겨찾기" class="icon-link favorite">
                            <i class="fas fa-heart"></i>
                        </a>
                        <a href="<c:url value='/user/logout'/>" title="로그아웃">
                            <i class="fas fa-sign-out-alt icon-link"></i>
                        </a>
                    </c:when>
                    <c:otherwise>
                        <a href="<c:url value='/user/index'/>" class="btn btn-sm btn-primary">로그인</a>
                        <a href="<c:url value='/user/register'/>" class="btn btn-sm btn-outline-gold">회원가입</a>
                    </c:otherwise>
                </c:choose>
            </div>
        </div>
    ...
    <nav class="user-nav">
        <div class="nav-container">
            <div class="nav-menu">
                <a href="<c:url value='/user/categoryStores'/>" class="nav-item ${empty param.categoryId ? 'active' : ''}">전체보기</a>
                
                <c:forEach var="category" items="${maincategoryList}" varStatus="status">
                ...
                <span class="nav-divider">|</span>
                <a href="<c:url value='/user/notice'/>" class="nav-item ${fn:contains(pageContext.request.requestURI, 'notice') ? 'active' : ''}">공지사항</a>
            </div>
        </div>
    </nav>
                    ...
                    <c:if test="${sessionScope.loginUser == null}">
                        <div class="auth-container">
                            ...
                            <div class="auth-form">
                                <form:form action="/user/login" method="post" modelAttribute="loginUser">
                                    <div class="form-group">
                                        ...
                                        <form:input path="user_id" id="id" class="form-control" placeholder="아이디를 입력하세요"/>
                            ...
                            <div class="auth-footer">
                                <p>계정이 없으신가요? <a href="<c:url value='/user/register'/>" class="auth-link">회원가입</a></p>
                            </div>
        ...
        <div class="footer-container">
            ...
            <div class="footer-column">
                ...
                <ul class="footer-links">
                    ...
                    <li class="footer-link"><a href="<c:url value='/user/categoryStores'/>">전체 가게</a></li>
                    <li class="footer-link"><a href="<c:url value='/user/notice'/>">공지사항</a></li>
                    <li class="footer-link"><a href="<c:url value='/user/mypage'/>">마이페이지</a></li>
                </ul>
            </div>
