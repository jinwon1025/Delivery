...
<c:choose>
	<c:when test="${not empty storeList}">
		<div class="store-list">
			<c:forEach items="${storeList}" var="store">
				<div
					class="store-item ${store.store_status == 0 ? 'store-closed' : ''}">
					<div class="store-logo">
						...
						<c:if test="${store.store_status == 0}">
							<div class="store-status-banner">영업 준비중</div>
						</c:if>
					</div>
					...
					<button class="favorite-btn"
						onclick="bookmarkStore(event, '${store.store_id}');">
						<c:choose>
	...
	<c:otherwise>
		<div class="no-stores">
			<i class="fas fa-store-slash mb-3"
...
<!-- 즐겨찾기 추가를 위한 폼 -->
<form id="addBookmarkForm" action="<c:url value='/user/addBookmark'/>"
	method="post">
...
<!-- 즐겨찾기 삭제를 위한 폼 -->
<form id="removeBookmarkForm"
	action="<c:url value='/user/removeBookmark'/>" method="post">
	<input type="hidden" id="remove_bm_store_id" name="bm_store_id"
		value=""> <input type="hidden" name="loginStatus"
		value="${sessionScope.loginUser.user_id}">
</form>
