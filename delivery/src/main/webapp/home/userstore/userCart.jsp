                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="empty-cart">
                            ...
                            <a href="/user/categoryStores" class="btn btn-primary" onclick="return checkLogin()">
            ...
            <c:otherwise>
                                                    ...
                                                    <c:if test="${option.MENU_ITEM_ID == item.MENU_ITEM_ID && option.ORDER_OPTION_ID == item.ORDER_OPTION_ID}">
                                                        ...
                                                        <c:forEach items="${optionNames}" var="name" varStatus="optStatus">
                                                            <div class="option-item">
                                                                ${name}
                                                                <c:if test="${not empty optionPrices[optStatus.index] && optionPrices[optStatus.index] != '0'}">
                                                                    <span>(+${optionPrices[optStatus.index]}원)</span>
                                                                </c:if>
                                                                <c:if test="${empty optionPrices[optStatus.index] || optionPrices[optStatus.index] == '0'}">
                                                                    <span>(+0원)</span>
                                                                </c:if>
                                                            </div>
                                                        </c:forEach>
                ...
                <div class="summary-card">
                    <div class="summary-header">
                        <h2><i class="fas fa-clipboard-list"></i> 주문 요약</h2>
                    </div>
                                ...
                                <span id="menuPriceDisplay">
                                    ...
                                    <c:forEach items="${cartDetails}" var="item">
                                        ...
                                        <c:if test="${!fn:contains(processedItems, itemKey)}">
                                            ...
                                            <c:set var="totalPrice" value="${totalPrice + ((item.MENU_PRICE + item.TOTAL_OPTION_PRICE) * itemQuantity)}" />
                ...
                <div class="action-buttons">
                    <a href="/userstore/returnToStore?store_id=${cartDetails[0].STORE_ID}" 
                        class="btn btn-outline btn-continue" onclick="return checkLogin()">
                        <i class="fas fa-arrow-left"></i> 메뉴 추가하기
                    </a>
                    <form action="/userStore/startOrder" method="get" onsubmit="return checkLogin()" style="flex: 2;">
                        ...
                        <c:forEach items="${cartDetails}" var="item" varStatus="status">
                            <c:if test="${status.index == 0 || cartDetails[status.index-1].MENU_ITEM_ID != item.MENU_ITEM_ID || cartDetails[status.index-1].ORDER_OPTION_ID != item.ORDER_OPTION_ID}">
                                <input type="hidden" name="selectedItems" value="${item.MENU_ITEM_ID}">
                                <input type="hidden" name="itemQuantities" value="${empty item.QUANTITY ? 1 : item.QUANTITY}">
                                <input type="hidden" name="orderOptionIds" value="${item.ORDER_OPTION_ID}">
                            </c:if>
                        </c:forEach>
